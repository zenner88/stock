...
//Add new pos sale
   ...
   function index()
   {
		...
		if ($this->form_validation->run() == true)
		{
			...
			$inv_total_no_tax = 0;
				...
				for($i=1; $i<=500; $i++){
					if( $this->input->post($quantity.$i) && $this->input->post($product.$i) && $this->input->post($unit_price.$i) ) {
						...
						$status_prin = $this->input->post($status.$i);
			...
			$keys = array("product_id","product_code","product_name","product_unit", "tax_rate_id", "tax","quantity","unit_price", "gross_total", "val_tax", "serial_no", "discount_val", "discount", "discount_id", "status_print");
		
			$items = array();
			foreach ( array_map(null, $product_id, $product_code, $product_name, $product_unit, $tax_rate_id, $tax, $inv_quantity, $inv_unit_price, $inv_gross_total, $val_tax, $serial, $val_ds, $discount, $dsID, $status_print) as $key => $value ) {
		...
		{ 
			if($suspend) {
				if($did) {
					...
					if($saleID = $this->pos_model->suspendSale($saleDetails, $items, $count, $did)) {
						$this->session->set_flashdata('success_message', $this->lang->line("sale_suspended"));
						redirect("module=pos&view=view_invoiceSus&id=".$did, 'refresh');
						// $this->pos_model->updateSuspend($did);			
						// redirect("module=waiters", 'refresh');
						// echo $did;	
						// if($this->pos_model->updateSale($did)) {
						// 	// redirect("module=pos", 'refresh');
						// 	echo $itemsdid;
						// }	
						// var_dump($items);
					...
					if($saleID = $this->pos_model->suspendSale($saleDetails, $items, $count, $did)) {
						$this->session->set_flashdata('success_message', $this->lang->line("sale_suspended"));
						redirect("module=pos&view=view_invoiceSus&id=".$saleID, 'refresh');
						// $this->pos_model->updateSuspend($saleID);			
						// redirect("module=pos", 'refresh');
						//echo $did;			
					}
				}
			} else {
				if($saleID = $this->pos_model->addSale($saleDetails, $items,$did)) {
	  ...
	  if($this->input->get('suspend_id')) {
		...
		$data['noMeja'] = $this->pos_model->getSaleByID($this->input->get('suspend_id'));		
		$data['waiter'] = $this->pos_model->getAllWaiters();		
	  } else {
	  ...
	  if(TAX2) {
	   ...
	   $data['subCategories'] = $this->possubcategories();
   ...
   function possubcategories($category_id = NULL) {
	
	if($this->input->get('category_id')) { $category_id = $this->input->get('category_id'); } else { $category_id = DCAT; }
	if($this->input->get('per_page') == 'n' ) { $page = 0; } else { $page = $this->input->get('per_page'); }
	
	$categories = $this->pos_model->getAllSubCategories();
	 $count = 1;
	 $cats = "";
	 foreach($categories as $category) {
...
//view inventory as html page
   ...
   function view_invoiceSus()
   {
	if($this->input->get('id')){ $sale_id = $this->input->get('id'); } else { $sale_id = NULL; }
	
	$data['message'] = (validation_errors() ? validation_errors() : $this->session->flashdata('message'));

		$data['rows'] = $this->pos_model->getAllInvoiceItemsSus($sale_id);
		
		$inv = $this->pos_model->getInvoiceBySaleIDSus($sale_id);
		$biller_id = $inv->biller_id;
		$customer_id = $inv->customer_id;
		$invoice_type_id = $inv->invoice_type;
		$data['biller'] = $this->pos_model->getBillerByID($biller_id);
   ...
   function Test()
   {
	...
	// echo $sale_id;
	// echo 1;
	$this->pos_model->updateSale($sale_id);			
	redirect("module=pos", 'refresh');
   ...
   function today_sale()
   {
	...
	// die();
	  ...
	  if($end_date = $this->input->post('end_date')){
		if($start_date = $this->input->post('start_date'))
		{
			
			if($cc_sale = $this->pos_model->getTodayCCSales($start_date,$end_date)) { $data['ccsales'] = $cc_sale->total; } else { $data['ccsales'] = "0.00"; }
			if($cash_sale = $this->pos_model->getTodayCashSales($start_date,$end_date)) { $data['cashsales'] = $cash_sale->total; } else { $data['cashsales'] = "0.00"; }
			if($ch_sale = $this->pos_model->getTodayChSales($start_date,$end_date)) { $data['chsales'] = $ch_sale->total; } else { $data['chsales'] = "0.00"; }
			
			$data['totalsales'] = $this->pos_model->getTodaySales($start_date,$end_date);
			$meta['page_title'] = $this->lang->line('today_sale');
			// if($cash_sale = $this->pos_model->getTodayCashSales($start_date,$end_date)) 
			// { 
	   ...
	   if($newData = $this->pos_model->getSaleByID($id)) {
		  ...
		  $sdata = "";
		   ...
		   $data = array(
					...
					'g_total' => $grand_total,
					'noMeja' => $this->input->post($newData->noMeja)					
					// 'g_total' => $grand_total
					);
		
					$data['noMeja'] = $this->pos_model->getSaleByID(1);
   ...
   function getSuspendedSales()
   {
	   ...
	   $this->datatables
			->select("suspended_bills.id as id, date, noMeja, namaCustomer, count, tax1, tax2, discount, total")
