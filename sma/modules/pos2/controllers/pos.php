...
//Add new pos sale
   ...
   function index()
   {
	  ...
	  if(TAX2) {
	   ...
	   $data['productssub'] = $this->ajaxproductsX(DCAT);	   
   ...
   function possubcategories($subcategory_id = NULL) {
	
	if($this->input->get('subcategory_id')) { $subcategory_id = $this->input->get('subcategory_id'); } else { $subcategory_id = DCAT; }
	if($this->input->get('per_page') == 'n' ) { $page = 0; } else { $page = $this->input->get('per_page'); }
	
	 $subcategories = $this->pos_model->getAllSubCategories();
	 $count = 1;
	 $cats = "";
	 foreach($subcategories as $subcategory) {
 
		 $cats .= "<li><button id=\"subcategory\" type=\"button\" value='".$subcategory->id."' class=\"gray\">
		 ".$subcategory->name."</button></li>";
		 $count++;
 ...
 if($this->input->get('per_page')) {
   ...
   function ajaxproductsX( $subcategory_id = NULL ) {
 ...
 $this->load->library("pagination");
	 ...
	 $config = array();
	 $config["base_url"] = base_url() . "index.php?module=pos&view=ajaxproducts";
	 $config["total_rows"] = $this->pos_model->products_count($subcategory_id);
	 $config["per_page"] = PLIMIT;
	 $config['prev_link'] = FALSE;
	 $config['next_link'] = FALSE;
	 $config['display_pages'] = FALSE;
	 ...
	 $products = $this->pos_model->fetch_productssub($subcategory_id, $config["per_page"], $page);
	 $pro = 1;
	 $prods = "<div>";
	 foreach($products as $product) {
		 $count = $product->id;
		 if($count < 10) { $count = "0".($count /100) *100;  }
		 if($subcategory_id < 10) { $subcategory_id = "0".($subcategory_id /100) *100;  }
	 if($product->image == 'no_image.jpg') {
		 $prods .= "
		 <button id=\"product-".$subcategory_id.$count."\" type=\"button\" value='".$product->code."' class=\"green\" ><i><img src=\"assets/uploads/thumbs/default.png\"></i><span><span>".$product->name."</span></span></button>";
	 } else {
		 $prods .= "
		 <button id=\"product-".$subcategory_id.$count."\" type=\"button\" value='".$product->code."' class=\"green\" ><i><img src=\"assets/uploads/thumbs/".$product->image."\"></i><span><span>".$product->name."</span></span></button>";
	 }
		 $pro++;	
	 }
