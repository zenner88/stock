...
if($this->input->post('submit')) {
		  ...
		  /*if($this->input->post('name')){
		   ...
		   if($this->input->post('waiters')){
			$v .= "&waiters=".$this->input->post('waiters');
			}
			if($this->input->post('categories')){
				$v .= "&categories=".$this->input->post('categories');
				}
				if($this->input->post('subCategories')){
					$v .= "&subCategories=".$this->input->post('subCategories');
					}
...
<div class="control-group">
  <label class="control-label" for="categories">Kategori Menu</label>
  <div class="controls"> <?php 
	   		$s1[""] = "";
	   		foreach($categories as $categories){
				$s1[$categories->id] = $categories->name;
			}
			echo form_dropdown('categories', $s1, (isset($_POST['categories']) ? $_POST['categories'] : ""), 'class="span4" id="categories" data-placeholder="Pilih Kategori Menu"');  ?> </div>
</div>

<div class="control-group">
  <label class="control-label" for="subCategories">Sub Kategori Menu</label>
  <div class="controls"> <?php 
	   		$s2[""] = "";
	   		foreach($subCategories as $subCategories){
				$s2[$subCategories->id] = $subCategories->name;
			}
			echo form_dropdown('subCategories', $s2, (isset($_POST['subCategories']) ? $_POST['subCategories'] : ""), 'class="span4" id="subCategories" data-placeholder="Pilih Sub Kategori Menu"');  ?> </div>
</div>

<div class="control-group">
  <label class="control-label" for="waiters">Waiters</label>
  <div class="controls"> <?php 
	   		$sp[""] = "";
	   		foreach($waiters as $waiters){
				$sp[$waiters->id] = $waiters->nama;
			}
			echo form_dropdown('waiters', $sp, (isset($_POST['waiters']) ? $_POST['waiters'] : ""), 'class="span4" id="waiters" data-placeholder="Pilih Waiters"');  ?> </div>
</div>
<div class="control-group">
