...
*/
   ...
   function sales()
   {
	  ...
	  $data['warehouses'] = $this->reports_model->getAllWarehouses();
	  $data['waiters'] = $this->reports_model->getAllWaiters();
	  $data['categories'] = $this->reports_model->getAllCategories();
	  $data['subCategories'] = $this->reports_model->getAllSubCategories();
   ...
   function getSales()
   {
		...
		if($this->input->get('waiters')){ $waiters = $this->input->get('waiters'); } else { $waiters = NULL; }
		if($this->input->get('categories')){ $categories = $this->input->get('categories'); } else { $categories = NULL; }
		if($this->input->get('subCategories')){ $subCategories = $this->input->get('subCategories'); } else { $subCategories = NULL; }
	   ...
	   $this->datatables
			...
			->join('waiters', 'waiters.id=sales.pelayan', 'left')
			...
			if($waiters) { $this->datatables->like('sales.pelayan', $waiters); }
			if($categories) { $this->datatables->like('products.category_id', $categories); }
			if($subCategories) { $this->datatables->like('products.subcategory_id', $subCategories); }
			...
			//  if($categories) { $this->datatables->where('products.category_id', 1000000); }
